...
import app.Model.app.library.Blackboard;
...
public class TheSubscriberMQTT implements Runnable, MqttCallback {
    ...
    public void run() {
        try (MqttClient client = new MqttClient(broker, clientID);){
            ...
            app.library.Blackboard.getInstance().reportMQTTBrokerError("Unable to connect to broker --\n\t" + e.getMessage());
    ...
    public void messageArrived(String s, MqttMessage mqttMessage) {
        app.library.Blackboard.getInstance().addClientData(topicAndPrefixPairs.get(s) + "~" + mqttMessage);
